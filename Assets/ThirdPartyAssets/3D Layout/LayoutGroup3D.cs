...
public class LayoutGroup3D : MonoBehaviour
{
    ...
    public void LinearLayout()
    {
        ...
        int elementCount = LayoutElements.Count;
        float totalLength = (elementCount - 1) * (ElementDimensions.x + Spacing); // Calculate the total length of all elements

        // Calculate the starting position offset based on the total length
        var startPositionOffset = StartPositionOffset - new Vector3(totalLength / 2f, 0f, 0f);
